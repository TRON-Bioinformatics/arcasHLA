...
def process_counts(count_file, eq_file, gene_list, allele_idx, allele_lengths):
    ...
    for eq, indices in eqs.items():
        ...
        genes = list(
            {get_gene(allele) for idx in indices for allele in allele_idx[idx]}
        )
        count = counts[eq]
...
def process_partial_counts(
    count_file, eq_file, allele_idx, allele_lengths, exon_idx, exon_combos
):
    """Processes pseudoalignment output, returning compatibility classes."""
    ...
    for eq, indices in eqs.items():
        ...
        genes = list(
            {allele.split("*")[0] for index in indices for allele in allele_idx[index]}
        )
        ...
        if len(genes) == 1 and count > 0:
            ...
            for exon in exons:
                exon_indices = list(
                    {index for index in indices if exon_idx[index] == exon}
                )
...
def get_count_stats(eq_idx, gene_length):
    ...
    stats = {gene: [0, 0, 0.0] for gene in eq_idx}
...
def alignment_summary(align_stats, partial=False):
    ...
    log.info(
        "[alignment] Processed {:.0f} reads, {:.0f} pseudoaligned ".format(
            total, count_unique + count_multi
        )
        + "to HLA reference"
    )

    log.info(
        "[alignment] {:.0f} reads mapped to a single HLA gene".format(count_unique)
    )
...
def get_alignment(
    ...
    total = pseudoalign(fqs, sample, paired, reference, outdir, temp, threads, avg, std)
    ...
    if partial:
        ...
        eq_idx, align_stats = process_partial_counts(
            count_file, eq_file, allele_idx, lengths, exon_idx, exon_combos
        )
...
def load_alignment(file, commithash, partial=False):
    ...
    if len(alignment_info) != 5:
        if partial:
            (commithash_alignment, eq_idx, paired, _, _, _, _) = alignment_info
            alignment_info = [commithash, eq_idx, None, paired, None, None]
        else:
            (
                commithash_alignment,
                eq_idx,
                allele_eq,
                paired,
                align_stats,
    ...
    if commithash != commithash_alignment:
        sys.exit(
            "[alignment] Error: reference used for alignment "
