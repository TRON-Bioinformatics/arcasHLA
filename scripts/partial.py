...
def filter_eqs(complete_genotypes, allele_idx, eq_idx, partial_alleles):
    ...
    all_predicted = {
        allele for alleles in complete_genotypes.values() for allele in alleles
    }
...
def type_partial(
    ...
    results = expectation_maximization(
        eqs[binding_region][gene],
        lengths,
        allele_idx,
        population,
        prior,
    ...
    overall = []
    for group in sorted(exon_groups.keys(), key=lambda x: len(x), reverse=False):
        ...
        explained_reads = {
            key: value for key, value in explained_reads.items() if value == top_perc
        }
        ...
        if population and len(explained_reads) > 1:
            ...
            for a1, a2 in explained_reads.keys():
                if (
                    process_allele(a1, 2) not in prior
                    or process_allele(a2, 2) not in prior
                ):
                    continue

                pair_prior[(a1, a2)] = (
                    prior[process_allele(a1, 2)][population]
                    * prior[process_allele(a2, 2)][population]
                )

            if pair_prior:
                a1, a2 = sorted(pair_prior.items(), key=lambda x: x[1], reverse=True)[
                    0
                ][0]
            else:
                a1, a2 = sorted(
    ...
    if overall:
        return sorted(overall, key=lambda x: (x[1], x[0][0], x[0][1]), reverse=True)[0][
            0
        ]
...
def do_partial_genotyping(
    ...
    with open(partial_json, "r") as json_file:
