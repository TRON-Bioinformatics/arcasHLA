...
def expectation_maximization(
    ...
    def drop_alleles(
        eqs, abundances, drop_iterations, drop_threshold, iterations, converged
    ):
        if iterations == 1:
            abundances = {
                allele: abundance
    ...
    for idx, count in sorted(
        undivided_counts.items(), key=lambda x: x[1], reverse=True
    )[:10]:
    ...
    while iterations < max_iterations and not converged:
        ...
        if srss_v != 0:
            ...
            if step_min < 0:
                ...
                theta_prime = {
                    allele: value / total for allele, value in theta_prime.items()
                }
        ...
        theta0, eqs = drop_alleles(
            eqs, theta_prime, drop_iterations, drop_threshold, iterations, converged
        )
        iterations += 1
...
def predict_genotype(
    ...
    if len(em_results) > 1:
        ...
        for (a1, a2), count in sorted(
            explained_reads.items(), key=lambda x: x[1], reverse=True
        ):
            alleles = ", ".join(
                [
            ...
            log.info("\t\t{: <28}    {: >9.2f}%\t".format(alleles, count * 100))

        max_count = max(explained_reads.values())
        top_by_reads = {
            pair: count for pair, count in explained_reads.items() if count == max_count
        }
        ...
        if len(top_by_reads) > 1 and population:
            for a1, a2 in top_by_reads.keys():
                ...
                pair_prior[(a1, a2)] = (
                    prior[allele1][population] * prior[allele2][population]
                )

        if pair_prior:
            max_prior = max(pair_prior.values())
            pair_prior = {
                pair: prior for pair, prior in pair_prior.items() if prior >= max_prior
            }

            a1, a2 = sorted(pair_prior.keys(), key=lambda x: (x[0], x[1]))[0]

        else:
            a1, a2 = sorted(top_by_reads.items(), key=lambda x: x[1], reverse=True)[0][
                0
            ]
        ...
        if a1_count == a2_count == 0:
            log.info(
                "[genotype] Unable to distinguish " + "between minor and major alleles"
            )
            genotype = [a1, a2]
        elif a1_count == 0:
            log.info(
                "[genotype] Likely heterozygous: minor allele has no "
                + "nonshared reads"
            )
            genotype = [a2, a2]
        elif a2_count == 0:
            ...
            log.info(
                f"[genotype] Likely homozygous: minor/major "
            ...
            log.info(
                f"[genotype] Likely heterozygous: minor/major "
                + "nonshared count {:.2f}".format(
                    min(a1_count / a2_count, a2_count / a1_count)
                )
            )
...
def genotype_gene(
    ...
    genotype, pair_count = predict_genotype(
        eqs,
        allele_idx,
        allele_eq,
        em_results,
    ...
    log.info(
        "\n[genotype] Most likely genotype explaining {:.0f} reads:".format(pair_count)
    )
...
def build_arg_parser(super_parser=None, subcommand_name="genotype"):
    parser_args = {
    ...
    parser.set_defaults(
        run_function=lambda parsed_args: do_genotyping(
            parsed_args.file,
            parsed_args.genes,
